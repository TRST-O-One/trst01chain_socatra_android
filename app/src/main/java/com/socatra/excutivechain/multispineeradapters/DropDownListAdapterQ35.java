...
public class DropDownListAdapterQ35 extends BaseAdapter {
	...
	public void setSelected(String selected) {
		DropDownListAdapterQ35.selected = selected;
	}

	public DropDownListAdapterQ35(Context context, ArrayList<String> items,
                                  TextView tv) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(RiskAssessmentActivity.checkSelected35[position])
	...
	private void setText(int position1){
		if (!RiskAssessmentActivity.checkSelected35[position1]) {
			RiskAssessmentActivity.checkSelected35[position1] = true;
			selectedCount++;
		} else {
			RiskAssessmentActivity.checkSelected35[position1] = false;
			selectedCount--;
		}
		...
		if (selectedCount == 0) {
			...
			for (int i = 0; i < RiskAssessmentActivity.checkSelected35.length; i++) {
				if (RiskAssessmentActivity.checkSelected35[i] == true) {
			...
			for (int i = 0; i < RiskAssessmentActivity.checkSelected35.length; i++) {
				if (RiskAssessmentActivity.checkSelected35[i]) {
