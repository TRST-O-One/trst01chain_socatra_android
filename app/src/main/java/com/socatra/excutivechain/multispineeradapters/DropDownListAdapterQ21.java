...
public class DropDownListAdapterQ21 extends BaseAdapter {
	...
	public void setSelected(String selected) {
		DropDownListAdapterQ21.selected = selected;
	}

	public DropDownListAdapterQ21(Context context, ArrayList<String> items,
                                  TextView tv) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(RiskAssessmentActivity.checkSelected21[position])
	...
	private void setText(int position1){
		if (!RiskAssessmentActivity.checkSelected21[position1]) {
			RiskAssessmentActivity.checkSelected21[position1] = true;
			selectedCount++;
		} else {
			RiskAssessmentActivity.checkSelected21[position1] = false;
			selectedCount--;
		}
		...
		if (selectedCount == 0) {
			...
			for (int i = 0; i < RiskAssessmentActivity.checkSelected21.length; i++) {
				if (RiskAssessmentActivity.checkSelected21[i] == true) {
			...
			for (int i = 0; i < RiskAssessmentActivity.checkSelected21.length; i++) {
				if (RiskAssessmentActivity.checkSelected21[i]) {
