...
public class DropDownListAdapterQ40 extends BaseAdapter {
	...
	public void setSelected(String selected) {
		DropDownListAdapterQ40.selected = selected;
	}

	public DropDownListAdapterQ40(Context context, ArrayList<String> items,
                                  TextView tv) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(RiskAssessmentActivity.checkSelected40[position])
	...
	private void setText(int position1){
		if (!RiskAssessmentActivity.checkSelected40[position1]) {
			RiskAssessmentActivity.checkSelected40[position1] = true;
			selectedCount++;
		} else {
			RiskAssessmentActivity.checkSelected40[position1] = false;
			selectedCount--;
		}
		...
		if (selectedCount == 0) {
			...
			for (int i = 0; i < RiskAssessmentActivity.checkSelected40.length; i++) {
				if (RiskAssessmentActivity.checkSelected40[i] == true) {
			...
			for (int i = 0; i < RiskAssessmentActivity.checkSelected40.length; i++) {
				if (RiskAssessmentActivity.checkSelected40[i]) {
