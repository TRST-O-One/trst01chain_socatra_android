...
public class DropDownListAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(RiskAssessmentActivity.checkSelected[position])
	...
	private void setText(int position1){
		if (!RiskAssessmentActivity.checkSelected[position1]) {
			RiskAssessmentActivity.checkSelected[position1] = true;
			selectedCount++;
		} else {
			RiskAssessmentActivity.checkSelected[position1] = false;
			selectedCount--;
		}
		...
		if (selectedCount == 0) {
			...
			for (int i = 0; i < RiskAssessmentActivity.checkSelected.length; i++) {
				if (RiskAssessmentActivity.checkSelected[i] == true) {
			...
			for (int i = 0; i < RiskAssessmentActivity.checkSelected.length; i++) {
				if (RiskAssessmentActivity.checkSelected[i]) {
