...
public class DropDownListAdapterQ31 extends BaseAdapter {
	...
	public void setSelected(String selected) {
		DropDownListAdapterQ31.selected = selected;
	}

	public DropDownListAdapterQ31(Context context, ArrayList<String> items,
                                  TextView tv) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(RiskAssessmentActivity.checkSelected31[position])
	...
	private void setText(int position1){
		if (!RiskAssessmentActivity.checkSelected31[position1]) {
			RiskAssessmentActivity.checkSelected31[position1] = true;
			selectedCount++;
		} else {
			RiskAssessmentActivity.checkSelected31[position1] = false;
			selectedCount--;
		}
		...
		if (selectedCount == 0) {
			...
			for (int i = 0; i < RiskAssessmentActivity.checkSelected31.length; i++) {
				if (RiskAssessmentActivity.checkSelected31[i] == true) {
			...
			for (int i = 0; i < RiskAssessmentActivity.checkSelected31.length; i++) {
				if (RiskAssessmentActivity.checkSelected31[i]) {
