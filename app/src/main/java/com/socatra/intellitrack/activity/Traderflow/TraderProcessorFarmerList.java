...
public class TraderProcessorFarmerList extends BaseActivity implements FarmerByDealerIdAdapter.OnItemClickListener {
...
//            searchByName.setMaxWidth(Integer.MAX_VALUE);
            ...
            SearchManager searchManager = (SearchManager) TraderProcessorFarmerList.this.getSystemService(Context.SEARCH_SERVICE);
            searchByName.setSearchableInfo(searchManager
                    .getSearchableInfo(TraderProcessorFarmerList.this.getComponentName()));
            searchByName.setMaxWidth(Integer.MAX_VALUE);
// listening to search query text change
        ...
        {
            getFarmerList(ProcessorId);
        }else {
            Toast.makeText(TraderProcessorFarmerList.this,"please check your internet connection",Toast.LENGTH_LONG).show();
        }

    }

    private void configureDagger() {
        AndroidInjection.inject(TraderProcessorFarmerList.this);
    }
    ...
    private void getFarmerList(String ProcessorId) {

        AppAPI service = Retrofit_funtion_class.getClient().create(AppAPI.class);

        final ProgressDialog progressDialog = new ProgressDialog(TraderProcessorFarmerList.this, R.style.AppCompatAlertDialogStyle);
        progressDialog.setCancelable(false);
        progressDialog.setMessage("loading data...");
        progressDialog.setProgressStyle(ProgressDialog.STYLE_SPINNER);
        progressDialog.show();

        Call<JsonElement> callRetrofit = service.getFarmerListbyProcessorIdFromserver(ProcessorId,  appHelper.getSharedPrefObj().getString(AUTHORIZATION_TOKEN_KEY, ""));
...
//                            }
                        ...
                        Toast.makeText(TraderProcessorFarmerList.this, "no records found", Toast.LENGTH_LONG).show();
                    }
                } catch (Exception ex) {
                    progressDialog.dismiss();
